...
export function WaitlistCTASection() {
  ...
  // Determine if this is from an anchor link by checking URL hash
  useEffect(() => {
    const checkForAnchor = () => {
      if (window.location.hash === '#notify-section' && !hasAnimated) {
        setHasAnimated(true)
      }
    }
    
    window.addEventListener('hashchange', checkForAnchor)
    checkForAnchor() // Check on initial load
    
    return () => window.removeEventListener('hashchange', checkForAnchor)
  }, [hasAnimated])
